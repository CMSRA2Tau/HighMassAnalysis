...
process.analyzeHiMassTau = cms.EDAnalyzer('HiMassTauAnalysis',
    ...
    RecoMuonTrackIsoTrkThreshold = cms.double(1.5),					# isolation tracks are required to have pt>X
    RecoMuonEcalIsoRecHitThreshold = cms.double(2.0),					# isolation rechits are required to have pt>X
...
#    DoRecoElectronDiscrByTrackerDrivenSeed = cms.bool(False),				# if true, require electron to be tracker driven
    ...
    DoDiscrByFirstLeadingJet		= cms.bool(False),
    ...
    DoDiscrBySecondLeadingJet		= cms.bool(False),
    ...
    RemoveFirstLeadingJetOverlapWithMuons           = cms.bool(True),
    ...
    RemoveSecondLeadingJetOverlapWithMuons           = cms.bool(True),
    ...
    RecoBJetEtaMaxCut                    = cms.double(999.4),                            # require jet |eta|<=X
    RecoBJetPtCut                        = cms.double(15.0),                             # require jet pt>=X
    RemoveBJetOverlapWithMuons           = cms.bool(True),                              # if true, jets w/ dR(muon,jet)<X will not be considered
    BJetMuonMatchingDeltaR               = cms.double(0.3),                              # dR(muon,jet)<X used for removing jets from the "good jet" list
    RemoveBJetOverlapWithElectrons       = cms.bool(False),                              # if true, jets w/ dR(electron,jet)<X will not be considered
    ...
    JetBTaggingTCHEcut                  = cms.double(1.7),                              # tagged as b-jet if TCHE > X
    ...
    RecoFirstLeadingJetNmin = cms.int32(0),
    RecoSecondLeadingJetNmin = cms.int32(0),
    RecoBJetNmin = cms.int32(1),							# require event to have >=X "jets" passing specified cuts
